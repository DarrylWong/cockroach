...
`
	...
	addFilterCmd = `
sudo tc class add dev %[1]s parent 1: classid 1:%[2]d htb rate 1000mbit
sudo tc qdisc add dev %[1]s parent 1:%[2]d handle %[3]d: netem delay %[4]s
sudo tc filter add dev %[1]s parent 1: protocol ip u32 match ip dst {ip:%[5]d}/32 flowid 1:%[2]d
sudo tc filter add dev %[1]s parent 1: protocol ip u32 match ip dst {ip:%[5]d:public}/32 flowid 1:%[2]d
`
	removeFilterCmd = `
